...
export class AppService {
    ...
    return new Promise<ICategory[]>((resolve, reject) => {
      request(`${this.URI}rus/`, (err, res, body) => {
        ...
        $('.product-item').each((i, el) => {
          const link = `https://greenwaystart.com${$('.product-item>a').eq(i).attr('href')}`
          const logo = `https://greenwaystart.com${$('.brand-logo').eq(i).attr('src')}`
          const title = $('.product-title').eq(i).text()
          const description = $('.product-info>p').eq(i).text()
          const img = `https://greenwaystart.com${$('.product-img').eq(i).attr('src')}`
          categories.push({
            link, logo, title, description, img,
          })
        })
        resolve(categories)
      })
    ...
    return new Promise<IItem[]>((resolve, reject) => {
      ...
      request(url, (err, res, body) => {
        ...
        $('.catalog-item').each((i, el) => {
          const link = `${this.URI}${$('.wrap-catalog-img>a').eq(i).attr('href')}`
          const img = `${this.URI}${$('.catalog-item-img>img').eq(i).attr('src')}`
          ...
          items.push({
            link, img, article, title, description, v, p,
          })
        })
        resolve(items)
      })
  ...
  async getItemInfo(url: string): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      request(url, (err, res, body) => {
        ...
        const title = $('h1').text()
        const description = $('#description').html().replace(/[\n\t]/g, '').trim()
  ...
  private getValFromItemInfo(arr: string[], col: string): number {
    return arr.map(el => el.toLowerCase().includes(col) ? +el.replace(/\D/g, '') : 0)
      .filter(e => e !== 0)[0] || 0
  }
}
